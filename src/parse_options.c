...
** Last update Wed Jun  5 12:22:54 2013 etienne bondot
*/
...
{
  if (!strcmp(av[2], "--encrypt") && strlen(av[2]) == strlen("--encrypt"))
    args->opt = ENCRYPT;
  else if (!strcmp(av[2], "--decrypt") && strlen(av[2]) == strlen("--decrypt"))
    args->opt = DECRYPT;
  else
